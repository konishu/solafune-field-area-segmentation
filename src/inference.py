...
def inference(model_path, image_path, output_path, device="cuda"):
    ...
    # 1. Load the model
    model = UNet(
        backbone_name="maxvit_small_tf_512.in1k", pretrained=False, num_classes=3
    ...
    cv2.imwrite(
        f"/workspace/projects/solafune-field-area-segmentation/outputs/ex0/output_images/{image_path.split('/')[-1].replace('.tif', '')}.png",
        (image[0, 0].cpu().numpy() * 255).astype(np.uint8),
    )
    ...
    if "original_shape" in locals():
        ...
        if (
            mask.ndim == 2 and original_shape[1] > 0 and original_shape[0] > 0
        ):  # If resize resulted in 2D mask, stack it
...
if __name__ == "__main__":
    ...
    image_path = (
        "/workspace/projects/solafune-field-area-segmentation/data/test_images/test_0.tif"  # Path to your test image
    )
