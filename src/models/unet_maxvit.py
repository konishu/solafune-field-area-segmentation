...
class UNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.decoder4(torch.cat([
            F.interpolate(x, size=enc_feats[-2].shape[2:], mode='bilinear', align_corners=False),
            enc_feats[-2]
        ], dim=1))

        x = self.decoder3(torch.cat([
            F.interpolate(x, size=enc_feats[-3].shape[2:], mode='bilinear', align_corners=False),
            enc_feats[-3]
        ], dim=1))

        x = self.decoder2(torch.cat([
            F.interpolate(x, size=enc_feats[-4].shape[2:], mode='bilinear', align_corners=False),
            enc_feats[-4]
        ], dim=1))

        x = self.decoder1(torch.cat([
            F.interpolate(x, size=enc_feats[-5].shape[2:], mode='bilinear', align_corners=False),
            enc_feats[-5]
        ], dim=1))
